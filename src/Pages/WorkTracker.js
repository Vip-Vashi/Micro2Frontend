...
const TechnicianTasks = () => {
  ...
  const [availability, setAvailability] = useState(''); // Dropdown state for availability
  const [selectedStatus, setSelectedStatus] = useState(''); // Selected status for filtering
  const [billingModalOpen, setBillingModalOpen] = useState(false); // Billing modal visibility
  const [serviceCharge, setServiceCharge] = useState(0);
  const [sparePartCharge, setSparePartCharge] = useState(0);
  ...
  const handleStatusUpdate = async (taskId) => {
    ...
    formData.append("sts", status);
    if (status === 'Pending') {
      formData.append("remarks", remarks); // Include remarks if status is Pending
      try {
        await axios.put('http://localhost:8000/servicerequests/stspending', formData);
        setTasks(prevTasks => prevTasks.map(task =>
          task.reqid === taskId ? { ...task, status, remarks: status === 'Pending' ? remarks : task.remarks } : task
      ...
      setBillingModalOpen(true); // Open billing modal for completed status
    } else {
      try {
        await axios.put('http://localhost:8000/servicerequests/sts', formData);
        setTasks(prevTasks => prevTasks.map(task =>
  ...
  const handleBillingSubmit = async () => {
    try {
      const totalBill = parseInt(serviceCharge) + parseInt(sparePartCharge);

      setBillAmount(totalBill);
       const status = "Pending";
      const data = new FormData();
      data.append("reqid",selectedTask.reqid);
      data.append("servicecharge",serviceCharge);
      data.append("sparepartcharge",sparePartCharge);
      data.append("billamt",parseInt(serviceCharge)+parseInt(sparePartCharge));
      data.append("status",status);
      // Update the task with billing details and set status to completed
      await axios.post('http://localhost:8000/payment', data);

      setTasks(prevTasks => prevTasks.map(task =>
        task.reqid === selectedTask.reqid
          ? { ...task, status: 'Completed', serviceCharge: parseFloat(serviceCharge), sparePartCharge: parseFloat(sparePartCharge), totalBill: totalBill }
          : task
      ));
      ...
      setSparePartCharge('');
      setBillAmount('');
      alert('Task status updated to completed and billing details saved.');
    } catch (error) {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Your Tasks</h1>
      {technician && (
        <div className="bg-indigo-100 shadow-md rounded-lg p-4 mb-6">
          <h2 className="text-xl font-semibold mb-2">Technician Status</h2>
          <p className="text-gray-700">Name: {technician.tname}</p>
          <p className="text-gray-700">Availability: {technician.status}</p>
          <p className="text-gray-700">Current Tasks: {currentTaskCount}</p> {/* Updated count */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
        {filteredTasks.length > 0 ? filteredTasks.map(task => (
          <div key={task.reqid} className="bg-violet-100 shadow-md rounded-lg p-4 hover:shadow-lg transition-shadow duration-300">
            <h2 className="text-xl font-semibold">{task.servicetype}</h2>
            <p className="text-gray-700 mt-2">Product Name: {task.product.appliances.productname}</p>
            <p className="text-gray-700 mt-2">Product Model: {task.product.appliances.model}</p>
            <p className="text-gray-700 mt-2">Product Issue: {task.issuedes}</p>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-80">
            ...
            <label className="block mb-2">
              ...
              <input
                type="number"
                value={sparePartCharge}
                onChange={(e) => setSparePartCharge(e.target.value)}
                className="border rounded-lg p-2 w-full mt-1"
                min="0"
              />
            </label>
            <p className="mt-2 font-semibold">Total Bill:{parseInt(serviceCharge)+parseInt(sparePartCharge)} {billAmount}</p>
            <button
